...
export function generateStaticParams() {
  return houseArchiveItems
    .filter((house) => !["ambre", "ambre-sans-faitage", "maison-calme", "asebra", "boreale"].includes(house.slug))
    .map((house) => ({
      slug: house.slug
    }));
}
...
export default async function HouseDetailPage({ params }: HouseDetailPageProps) {
  ...
  if (!house || ["ambre", "ambre-sans-faitage", "maison-calme", "asebra", "boreale"].includes(house.slug)) {
    notFound();
  }
