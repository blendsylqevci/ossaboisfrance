...
function house_starting_price_shortcode($atts) {
    ...
    return '<span style="font-size: 16px; font-weight: 400; color: inherit;">À partir de</span> ' .
           '<strong style="font-size: 22px; font-weight: bold; color: #1B1B1B;">' .
           esc_html($formatted_price) . '&nbsp;€</strong>';
}
    ...
    <div class="order-sections">
            ...
            <table class="order-details-table">
                ...
                <tr>
                    ...
                    <td><?php
                        ...
                        echo esc_html($size);
                        if ($size_price) echo ' (€' . esc_html($size_price) . ')';
                    ?></td>
                </tr>
                <tr>
                    ...
                    <td><?php
                        ...
                        if ($iso_price) echo ' (€' . esc_html($iso_price) . ' per m²)';
                    ?></td>
                </tr>
                <tr>
                    ...
                    <td><?php
                        ...
                        if ($inner_iso_price) echo ' (€' . esc_html($inner_iso_price) . ' per m²)';
                    ?></td>
                </tr>
                <tr>
                    <td>Facade:</td>
                    <td><?php
                        ...
                        if ($facade_price) echo ' (€' . esc_html($facade_price) . ' per m²)';
                    ?></td>
                </tr>
                <tr>
                    <td>Roof:</td>
                    <td><?php
                        ...
                        if ($roof_price) echo ' (€' . esc_html($roof_price) . ' per m²)';
                    ?></td>
                </tr>
                <tr>
                    ...
                    <td><?php
                        ...
                        if ($struktura_price) echo ' (€' . esc_html($struktura_price) . ')';
                    ?></td>
                </tr>
                <tr>
                    ...
                    <td><?php
                        ...
                        if ($dritaret_price) echo ' (€' . esc_html($dritaret_price) . ')';
                    ?></td>
                </tr>
            </table>
        </div>
        ...
        <div class="order-section">
            ...
            <table class="order-details-table">
                <tr>
                    <td>Base Price:</td>
                    <td>€<?php echo number_format(floatval(get_post_meta($post->ID, 'base_price', true) ?: 0), 2, '.', ','); ?></td>
                </tr>
                ...
                <tr>
                    ...
                    <td>€<?php echo number_format($transportation_cost, 2, '.', ','); ?></td>
                </tr>
                <?php endif; ?>
                <tr>
                    <td>Total Price:</td>
                    <td class="order-price">€<?php echo number_format(floatval(get_post_meta($post->ID, 'total_price', true) ?: 0), 2, '.', ','); ?></td>
                </tr>
            </table>
        </div>
        <?php
        ...
        ?>
        <div class="order-section">
            <h3>Përdhesa (Measurements)</h3>
            <table class="order-details-table">
                <tr><td>Sipërfaqja Bruto:</td><td><?php echo esc_html($perdhesa['bruto'] ?? '0'); ?> m²</td></tr>
                <tr><td>Sipërfaqja Neto:</td><td><?php echo esc_html($perdhesa['neto'] ?? '0'); ?> m²</td></tr>
                <tr><td>Mure te Jashtme:</td><td><?php echo esc_html($perdhesa['mure_te_jashtme'] ?? '0'); ?> m²</td></tr>
                <tr><td>Mure Mbajtese:</td><td><?php echo esc_html($perdhesa['mure_mbajtese'] ?? '0'); ?> m²</td></tr>
                <tr><td>Mure Ndarese:</td><td><?php echo esc_html($perdhesa['mure_ndarese'] ?? '0'); ?> m²</td></tr>
                <tr><td>Pllaka e Kulmit:</td><td><?php echo esc_html($perdhesa['pllaka_e_kulmit'] ?? '0'); ?> m²</td></tr>
            </table>
        </div>
        <?php
            }
...
function handle_house_order_submission_updated() {
    ...
    $message .= '<tr style="background: #f5f5f5;"><td style="padding: 10px; font-weight: bold;">Subtotal:</td><td style="padding: 10px; text-align: right;">€' . number_format($subtotal, 2, '.', ',') . '</td></tr>';
    $message .= '<tr><td style="padding: 10px; font-weight: bold;">Base Price:</td><td style="padding: 10px; text-align: right;">€' . number_format($base_price, 2, '.', ',') . '</td></tr>';

    if (isset($selections['priceBreakdown']) && is_array($selections['priceBreakdown'])) {
        foreach ($selections['priceBreakdown'] as $item) {
            $label = sanitize_text_field($item['label'] ?? '');
            $value = floatval($item['value'] ?? 0);
            $message .= '<tr><td style="padding: 10px;">' . esc_html($label) . ':</td><td style="padding: 10px; text-align: right;">€' . number_format($value, 2, '.', ',') . '</td></tr>';
        }
    }

    $message .= '<tr><td style="padding: 10px; font-weight: bold;">Transportation:</td><td style="padding: 10px; text-align: right;">€' . number_format($transportation_cost, 2, '.', ',') . '</td></tr>';

    $message .= '<tr style="background: #1F4B43; color: white;"><td style="padding: 10px; font-weight: bold;">Total:</td><td style="padding: 10px; text-align: right; font-weight: bold; font-size: 18px;">€' . number_format($total_price, 2, '.', ',') . '</td></tr>';
    $message .= '</table>';

    // Perdhesa
    if (isset($selections['perdhesa']) && is_array($selections['perdhesa'])) {
        $perdhesa = $selections['perdhesa'];
        $message .= '<h3>Përdhesa (Measurements)</h3>';
        $message .= '<table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">';
        $message .= '<tr><td style="padding: 10px; font-weight: bold;">Sipërfaqja Bruto:</td><td style="padding: 10px;">' . esc_html($perdhesa['bruto'] ?? '0') . ' m²</td></tr>';
        $message .= '<tr><td style="padding: 10px; font-weight: bold;">Sipërfaqja Neto:</td><td style="padding: 10px;">' . esc_html($perdhesa['neto'] ?? '0') . ' m²</td></tr>';
        $message .= '<tr><td style="padding: 10px; font-weight: bold;">Mure te Jashtme:</td><td style="padding: 10px;">' . esc_html($perdhesa['mure_te_jashtme'] ?? '0') . ' m²</td></tr>';
        $message .= '<tr><td style="padding: 10px; font-weight: bold;">Mure Mbajtese:</td><td style="padding: 10px;">' . esc_html($perdhesa['mure_mbajtese'] ?? '0') . ' m²</td></tr>';
        $message .= '<tr><td style="padding: 10px; font-weight: bold;">Mure Ndarese:</td><td style="padding: 10px;">' . esc_html($perdhesa['mure_ndarese'] ?? '0') . ' m²</td></tr>';
        $message .= '<tr><td style="padding: 10px; font-weight: bold;">Pllaka e Kulmit:</td><td style="padding: 10px;">' . esc_html($perdhesa['pllaka_e_kulmit'] ?? '0') . ' m²</td></tr>';
        $message .= '</table>';
    }
    ...
    if (!empty($email) && is_email($email)) {
        ...
        $customer_message .= '<table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">';
        $customer_message .= '<tr><td style="padding: 10px; font-weight: bold;">Subtotal:</td><td style="padding: 10px; text-align: right;">€' . number_format($subtotal, 2, '.', ',') . '</td></tr>';
        $customer_message .= '<tr><td style="padding: 10px; font-weight: bold;">Transportation:</td><td style="padding: 10px; text-align: right;">€' . number_format($transportation_cost, 2, '.', ',') . '</td></tr>';
        $customer_message .= '<tr style="background: #1F4B43; color: white;"><td style="padding: 10px; font-weight: bold;">Total:</td><td style="padding: 10px; text-align: right; font-weight: bold; font-size: 18px;">€' . number_format($total_price, 2, '.', ',') . '</td></tr>';
        $customer_message .= '</table>';
